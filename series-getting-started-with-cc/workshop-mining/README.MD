...
<div align="center" padding=25px>
    <img src="images/overview-ws-aws.png" width=100% height=100%>
</div>
...
10. [Step 8 — Flink Agentic AI (LLM Inference)](#step-8--flink-agentic-ai-llm-inference)
    - [8.1 Prepare LLM Access (AWS Bedrock)](#81-prepare-llm-access-aws-bedrock)
...
- Access to **AWS Console to get Credential for AWS Bedrock**.
- A modern browser.
...
3. Under **Payment details & contacts**, enter promo code **`will be provided`** to skip entering a credit card for 5 days (for lab purposes).
...
### 8.1 Prepare LLM Access (AWS Bedrock)
  1. Go to AWS Console → **Get AWS_ACCESS_KEY_ID, AWS_SECRET_ACCESS_KEY, AWS_SESSION_TOKEN  ** 
  2. Set AWS environment variables, e.g.  
      ```bash
     ...
     ```
  3. View available Bedrock models,  
  4. Get the Inference Profiles ID,  e.g. **apac.anthropic.claude-3-5-sonnet-20240620-v1:0**  or  **apac.anthropic.claude-3-7-sonnet-20250219-v1:0** 
...
3. Create the **Flink connection** (replace placeholders with your values where needed):
   ...
   confluent flink connection create <connection_name> \
     ...
     --region <region> \
     --type bedrock \
     --endpoint https://bedrock-runtime.<region>.amazonaws.com/model/<inference_profiles_id>/invoke \
     --aws-access-key $AWS_ACCESS_KEY_ID \
     --aws-secret-key $AWS_SECRET_ACCESS_KEY \
     --aws-session-token $AWS_SESSION_TOKEN
   ```
4. Example **Flink connection**:
   ```bash
   confluent flink connection create bedrock-connection \
     ...
     --aws-session-token $AWS_SESSION_TOKEN
   ```
5. If you need to delete the Flink connection, run the following command:
   ```bash
...
> Ensure the **credential, model id, region id ** matches what you noted in 8.1.
