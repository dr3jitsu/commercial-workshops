...
3. Under **Payment details & contacts**, enter promo code **`POPTOUT000EXB3W`** to delay entering a credit card for 30 days (for lab purposes).
...
- **Option A (Lab sample):**
  - **API Key:** `AIzaSyA4ggpgB0zyvugMP6Dpa8RmUUQB1DUNofI`
...
4. Example:
   ```bash
   confluent flink connection create mining-connection \
    --cloud AWS \
    --region ap-southeast-1 \
    --environment env-jwnnxq \
    --type googleai \
    --endpoint https://generativelanguage.googleapis.com/v1beta/models/gemini-2.0-flash:generateContent \
